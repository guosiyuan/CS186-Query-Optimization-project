...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int numdist = getNumDistinct();

    if (predicate == PredicateOperator.EQUALS) {
      reductionFactor = 1/(float) numdist;
    } else if (predicate == PredicateOperator.NOT_EQUALS) {
      reductionFactor = 1-(1/(float) numdist);
    } else if (predicate == PredicateOperator.LESS_THAN){
      //reductionFactor = ((float)getEntriesInRange(getMinValue(), value.getInt())/((float)getEntriesInRange(getMinValue(), getMaxValue())));
      reductionFactor = (((float)val - (float) minval)/((float)maxval-(float)minval));

    } else if (predicate == PredicateOperator.LESS_THAN_EQUALS) {
      reductionFactor = (((float)val - (float) minval)/((float)maxval-(float)minval)) + (float)(1/(float)numdist);
      
    } else if (predicate == PredicateOperator.GREATER_THAN) {
      reductionFactor = ((float)maxval - (float)val)/((float)maxval-(float)minval);
    } else if (predicate == PredicateOperator.GREATER_THAN_EQUALS) {
      reductionFactor = (((float)maxval - (float)val)/((float)maxval - (float) minval)) + (float)(1/(float)numdist);
    } else {
      reductionFactor = 1/10;
    }
